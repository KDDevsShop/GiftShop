...
function UserProfileForm() {
  ...
  }, [accessToken, fetchLoggedInUser, user]);
  ...
  const handleChangeAvatar = async () => {
    ...
    try {
      ...
      await userService.changeAvatar(avatarFile, accessToken);
      toast.success("Change successfully!");
      fetchLoggedInUser();

      const updatedUser = await userService.getLoggedInUser(accessToken);
      localStorage.setItem("user", JSON.stringify(updatedUser));
      setUser(updatedUser);
      setAvatarPreview(
        updatedUser?.avatarImagePath?.startsWith("http")
          ? updatedUser.avatarImagePath
          : `http://localhost:5000/${updatedUser.avatarImagePath?.replace(
              /\\/g,
              "/"
            )}`
      );
